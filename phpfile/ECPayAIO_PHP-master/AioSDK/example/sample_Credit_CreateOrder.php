...
// echo json_encode($datas[0]['title']);
// exit();
...
*/
    ...
    try {
        ...
        // $obj->Send['ReturnURL']         = "http://localhost/timevolts_pika/phpfile/ECPayAIO_PHP-master/AioSDK/example/sample_ServerReplyPaymentStatus.php" ;    //付款完成通知回傳的網址
        $obj->Send['MerchantTradeNo']   = $MerchantTradeNo;                          //訂單編號
        $obj->Send['MerchantTradeDate'] = date('Y/m/d H:i:s');                       //交易時間
        $obj->Send['TotalAmount']       = $datas["totalAmount"];                     //交易金額
        $obj->Send['TradeDesc']         = "good to drink" ;                          //交易描述
        $obj->Send['ChoosePayment']     = ECPay_PaymentMethod::Credit ;              //付款方式:Credit
        $obj->Send['IgnorePayment']     = ECPay_PaymentMethod::GooglePay ;           //不使用付款方式:GooglePay

        // $obj->Send['OrderResultURL']     = "http://localhost:8080/ProductCheckOut" ;           
        ...
        //            'Currency' => "元", 'Quantity' => (int) "10", 'URL' => "dedwed"));
        // array_push($obj->Send['Items'], array('Name' => "歐付寶黑芝麻豆漿", 'Price' => (int)"2000",
        //            'Currency' => "元", 'Quantity' => (int) "10", 'URL' => "dedwed"));
        
        foreach ($datas["cart"] as $key => $value) {
            array_push($obj->Send['Items'], array('Name' => $value["title"], 'Price' => (int)$value["price"] / (int)$value["amount"],
                   'Currency' => "元", 'Quantity' => (int)$value["amount"], 'URL' => "dedwed"));
        }
